...
// اختيار الـ port من الـ argument أو استخدام 3002 كـ default
const port = parseInt(process.argv[2]) || 3002;
const BLOCKCHAIN_FILE = `blockchain-${port}.json`; // ملف مختلف لكل نود
const DIFFICULTY = 4;
...
app.post("/mine", async (req, res) => {
  try {
    ...
    for (const tx of transactions) {
      ...
      const index = pendingTransactions.findIndex(t => t.from === tx.from && t.to === tx.to && t.amount === tx.amount && t.fee === tx.fee && t.signature === tx.signature);
